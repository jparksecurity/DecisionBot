...
def call_gmi_cloud(combined_text, api_key=None):
    ...
    if not api_key:
        # Try to get API key from direct environment variable first (for local testing)
        api_key = os.getenv("GMI_API_KEY")
        
        # If not found, try to get it from ByteNite job parameters
        if not api_key:
            try:
                app_params_raw = os.getenv('APP_PARAMS')
                if app_params_raw:
                    app_params = json.loads(app_params_raw)
                    api_key = app_params.get('GMI_API_KEY')
            except (json.JSONDecodeError, AttributeError):
                pass
    
    if not api_key:
        raise ValueError("GMI_API_KEY not found in environment or job parameters. Pass it via job params: {'params': {'app': {'GMI_API_KEY': 'your-key'}}}")
...
def main():
    ...
    # For ByteNite assemblers, use TASK_RESULTS_DIR and OUTPUT_DIR
    input_dir = os.getenv("TASK_RESULTS_DIR", "/input")
    output_file = os.path.join(os.getenv("OUTPUT_DIR", "/output"), "decisions.json")
    
    # Support legacy environment variables for backward compatibility
    if not os.getenv("TASK_RESULTS_DIR"):
